...
public class PoseSkeleton
{
    ...
    public Transform[] linkedKeypoints;

    //Z-offset for the skeleton to keep it in front of video feed
    public Transform displayOffset;
    // Point to keep all positions relative to
    // This will be the video screen, so joint positions can be scaled down
    ...
    private void InitialiseLine(int pairIndex, float width, Color colour)
    {
    ...
    private void InitializeSkeleton()
    {
        ...
        {
            InitialiseLine(i, lineWidth, colours[i]);
        }
    }
    
    public PoseSkeleton(Transform anchorPoint, float pointScale = 10f, float lineWidth = 5f)
    {
        ...
        for (int i = 0; i < NUM_KEYPOINTS; i++)
        {
            ...
            this.keypoints[i].parent = anchorPoint;

        }
        linkedKeypoints = (Transform[]) keypoints.Clone();
        ...
        float sourceScale, RenderTexture sourceTexture, bool mirrorImage, float minConfidence)
    {
        ...
        for (int k = 0; k < keypoints.Length; k++)
        {
            ...
            Vector3 coords = keypoints[k].position * sourceScale;
            ...
            this.keypoints[k].parent = displayOffset;

            // Update the current key point location
            // Set the z value to -1f to place it in front of the video screen
            this.keypoints[k].position = new Vector3(coords.x, coords.y, -1);
        }
        UpdateKeyPointsForModel(keypoints, sourceScale, sourceTexture, mirrorImage, minConfidence);
    }

    public void UpdateKeyPointsForModel(Utils.Keypoint[] keypoints,
        float sourceScale, RenderTexture sourceTexture, bool mirrorImage, float minConfidence)
    {
        // Iterate through the key points
        for (int k = 0; k < keypoints.Length; k++)
        {
            ...
            linkedKeypoints[k].parent = null;
            // Update the current key point location
            // Set the z value to -1f to place it in front of the video screen
            linkedKeypoints[k].position = new Vector3(coords.x, coords.y, coords.z);
